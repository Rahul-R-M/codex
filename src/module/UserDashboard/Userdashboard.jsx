...
const UserDashboard = () => {
  ...
  const onSubmit = (values) => {
    if (isEditMode) {
      // Edit the task
      const updatedTasks = UserTaskList?.map(task => task.taskid === values.taskid ? values : task);
      const userTask = userData?.map(item => +item.id === +id ? {...item, task: updatedTasks} : item)
      dispatch(editUserTask(userTask))
      
    } else {
      // Add new task
      const newTask = { ...values, taskid: Math.floor(Math.random() * 1000) };
      const userTask = userData?.map(item => +item.id === +id ? {...item, task: [...item.task, newTask]} : item)
      dispatch(addUserTask(userTask))
      // localStorage.setItem('Signupdata', JSON.stringify(userTask));
    }  
    resetForm();
    handleClose();
  };
  ...
  const UserTaskList = useMemo(()=>{
    return users?.filter(item => +item.id === +id)[0]?.task
  },[id, users])
